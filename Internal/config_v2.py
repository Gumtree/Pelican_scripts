...
if System.getProperty("gumtree.sics.useNewProxy") != 'true':
    load_script("config.py")

def slog(text):
    logger.log(text)
    
__listener_list__ = []
class __ValueListener__(control.ControllerAdapter):
    
    def __init__(self, par, options = None):
        self.par = par
...
#        slog(self.par.title + ' = ' + newValue.getStringData())
        if self.par.type == 'STRING':
            self.par.value = str(newValue)
        elif self.par.type == 'FLOAT':
            self.par.value = float(newValue)
        elif self.par.type == 'INT':
            self.par.value = int(newValue)
        elif self.par.type == 'BOOL':
            if not self.options is None:
                value = round(newValue)
                if value == options[1]:
                    self.par.value = True
                else:
                    self.par.value = False
            else:
                self.par.value = bool(round(newValue))
...
def init_par(par, title, cmd, path, options = None, option_values = None, bool_values = None):
    ...
    if path is None or len(path) == 0:
        return
    controller = control.get_controller(path)
    if controller != None:
        if options != None:
            value = round(controller.getValue())
            try:
            ...
            if par.type == 'STRING':
                par.value = str(controller.getValue())
            elif par.type == 'FLOAT':
                par.value = float(controller.getValue())
            elif par.type == 'INT':
                par.value = int(controller.getValue())
            elif par.type == 'BOOL':
                if bool_values != None:
                    value = round(controller.getValue())
                    ...
                    par.value = bool(round(controller.getValue()))
...
def par_changed(par):
    ...
    if controller != None:
        if par.options != None:
            value = float(controller.getValue())
            try:
                idx = par.options.index(par.value)
                new_value = eval(par.option_values)[idx]
                if value != new_value:
                    control.drive(path, new_value)
                    slog('drive ' + par.title + ' to ' + str(par.value))
            except:
                print 'failed to drive ' + path
        else:
            if par.type == 'STRING' :
                value = str(controller.getValue())
                if par.value != value:
                    controller.setTargetValue(str(par.value))
                    controller.commitTargetValue()
                    slog('set ' + par.title + ' to ' + str(par.value))
            elif par.type == 'FLOAT' :
                value = float(controller.getValue())
                if par.value != value:
                    control.drive(path, par.value)
                    slog('drive ' + par.title + ' to ' + str(par.value))
            elif par.value == 'INT':
                value = int(controller.getValue())
                if par.value != value:
                    control.drive(path, float(par.value))
                    slog('drive ' + par.title + ' to ' + str(par.value))
            elif par.type == 'BOOL':
                if par.bool_values != None:
                    value = round(controller.getValue())
                    if value != par.bool_values[int(par.value)] :
                        control.drive(path, par.bool_values[int(par.value)])
                        slog('set ' + par.title + ' to ' + str(par.value))
                else:
                    control.drive(path, int(par.value))
                    slog('set ' + par.title + ' to ' + str(par.value))
    else:
...
def __dispose__():
    ...
    for listener in __listener_list__:
        listener.controller.removeControllerListener(listener)
